...
class TestClassifyIntent:
    ...
    def test_returns_intent_result(
        ...
        result = classify_intent(
            ...
            available_templates=["shp2geojson", "clip_raster", "merge_shp"],
            history=[],
            client=client,
            builder=builder,
        )
    ...
    def test_invalid_template_id_sets_zero_confidence(
        ...
        result = classify_intent(
            user_input="随便说点什么",
            available_templates=["shp2geojson", "clip_raster"],
            history=[],
            client=client,
            builder=builder,
        )
    ...
    def test_passes_available_templates_to_prompt(
        self, client: MagicMock, builder: PromptBuilder
    ) -> None:
        """F2: Available templates are included in the prompt."""
        client.chat.return_value = json.dumps(
    ...
    def test_uses_low_temperature(
        ...
        classify_intent(
            user_input="test",
            available_templates=["shp2geojson"],
            history=[],
            client=client,
            builder=builder,
        )
