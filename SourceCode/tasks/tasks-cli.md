...
**依赖关系**：
```
main() → parse_args() → load_config() → workspace.initialize() → get_retriever()
...
- [ ] `tests/unit/test_cli_main.py`:
  ...
  - `test_main_rag_init_failure`: RAG 模型缺失 → 打印错误 → 返回 1
...
- `get_retriever()`: mock，部分测试抛 `RuntimeError`
...
- [ ] `cli/main.py`:
  - `main(argv: Optional[list[str]] = None) -> int`:
    ...
    4. `get_retriever()` → 失败 → 打印错误 → 返回 1
    5. 定位模板目录：`pkg_root / "data/templates/"`
    6. `scan_templates(template_dir)` → `List[TemplateDef]`
    7. `TemplateRegistry(templates, template_dir)`
    8. `ParamValidator(get_workspace())`
    9. `TemplateEngine(template_dir, get_workspace())`
    10. `LLMClient()`, `PromptBuilder(agents_md)`
    11. `SessionProcessor(registry, validator, template_engine, llm_client, prompt_builder)`
    12. `ScriptExecutor(get_workspace())`
    13. `REPL(processor, executor, dry_run=args.dry_run)`
    14. 打印欢迎信息（含工作空间路径、模板数量、`/help` 提示）
    15. `repl.run(Session())`
    16. 返回 0
