      ...
      {TABS.map((tab) => {
        ...
        return (
          <button
            ...
            className={`relative px-4 py-2.5 text-sm font-semibold flex items-center gap-1.5 transition-colors cursor-pointer border-none bg-transparent
          ...
          >
            ...
            {tab.id === 'qa' && qaMessageCount > 0 && (
              <span
                className={`text-2xs font-semibold px-[5px] py-[1px] rounded ${isActive ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}
              >
                {qaMessageCount}
              </span>
            )}
